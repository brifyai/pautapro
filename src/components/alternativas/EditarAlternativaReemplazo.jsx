    ...
    const loadSoportesParaPrograma = async (soporteId) => {
        try {
          ...
          setAlternativa(prev => ({
            ...
            soporte: soporteData.nombreidentificador,
            bonificacionano: soporteData.bonificacionano || 0,
 ...
 useEffect(() => {
  if (alternativa && alternativa.calendar) {
    ...
    const fetchTemasFiltrados = async () => {
        try {
            ...
            // Construir consulta base sin relaciones complejas
            let query = supabase
                .from('temas')
                .select(`
                  id_tema,
            ...
            const temasConRelaciones = await Promise.all(
                (data || []).map(async (tema) => {
                    let medioData = null;
                    let calidadData = null;
                    
                    // Obtener medio si tiene id_medio
                    if (tema.id_medio) {
                    ...
                    // Obtener calidad si tiene id_Calidad
                    if (tema.id_Calidad) {
                        const { data: calidad } = await supabase
                            .from('calidad')
                            .select('id, nombrecalidad')
                            .eq('id', tema.id_Calidad)
                            .single();
                        calidadData = calidad;
                    }
            ...
            console.log("Temas filtrados:", temasConRelaciones);
            setTemasFiltrados(temasConRelaciones || []);
        } catch (error) {
   ...
   const handleCantidadChange = (dia, valor) => {
    ...
    const handleSearchSoporte = (search) => {
        ...
        const filtrados = soportes.filter(soporte =>
            soporte.nombreidentificador.toLowerCase().includes(search.toLowerCase())
        );
    ...
    const fetchPlanInfo = async () => {
      try {
          // Obtener información de la orden sin relaciones complejas
          ...
          if (ordenData?.id_campania) {
              const { data: campania } = await supabase
                  .from('campania')
                  .select('id_campania, nombrecampania, id_Cliente')
                  .eq('id_campania', ordenData.id_campania)
                  .single();
              
              if (campania) {
                  campaniaData = campania;
                  
                  // Obtener información del cliente
                  if (campania.id_Cliente) {
                      const { data: cliente } = await supabase
                          .from('clientes')
                          .select('id_cliente, nombrecliente')
                          .eq('id_cliente', campania.id_Cliente)
                          .single();
                      clienteData = cliente;
                  }
              }
          ...
          if (ordenData?.id_plan) {
              ...
              if (!plan) {
                  console.error('Error fetching plan:', 'Plan not found');
              } else {
                  planData = plan;
              }
          }
          ...
          setPlanInfo({
              ...
              campania: campaniaData?.nombrecampania || 'No disponible',
              cliente: clienteData?.nombrecliente || 'No disponible',
              clienteId: campaniaData?.id_Cliente
          });
      } catch (error) {
  ...
  const fetchAlternativa = async () => {
    try {
        ...
        // Obtener contrato
        if (data.num_contrato) {
            const { data: contrato } = await supabase
                .from('contratos')
                .select('*')
                .eq('id', data.num_contrato)
                .single();
        ...
        // Parsear el calendario si existe
        if (alternativaCompleta.calendar) {
            try {
                let calendarArray = [];
                if (typeof alternativaCompleta.calendar === 'string') {
                    calendarArray = JSON.parse(alternativaCompleta.calendar);
                } else if (Array.isArray(alternativaCompleta.calendar)) {
                    calendarArray = alternativaCompleta.calendar;
                }
                
                // Actualizar el estado de cantidades en el objeto alternativa
                alternativaCompleta.cantidades = calendarArray;
                ...
                console.error('Error al parsear el calendario:', error);
                alternativaCompleta.cantidades = [];
                setCalendarData([]);
            }
        } else {
            alternativaCompleta.cantidades = [];
        ...
        setAlternativa(alternativaCompleta);
        
        // Obtener información del plan
        if (alternativaCompleta.id_orden) {
            fetchPlanInfo(alternativaCompleta.id_orden);
        }
    } catch (error) {
            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={6}>
                    <TextField
                        ...
                        margin="normal"
                        label="Soporte"
                        value={alternativa.Soportes?.nombreidentificador || ''}
                        InputProps={{
                            readOnly: true,
                            startAdornment: (
                                <InputAdornment position="start">
