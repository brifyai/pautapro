...
const ViewCampania = () => {
    ...
    const fetchCampanaDetails = async () => {
        try {
            ...
            const [clientesData, productosData, aniosData, agenciasData] = await Promise.all([
                supabase.from('clientes').select('id_cliente, nombrecliente').eq('id_cliente', data.id_cliente),
                supabase.from('productos').select('id, nombredelproducto').eq('id', data.id_producto),
                supabase.from('anios').select('id, years').eq('id', data.id_anio),
                supabase.from('agencias').select('id, nombreidentificador').eq('id', data.id_agencia)
            ]);

            const campanaConRelaciones = {
                ...data,
                clientes: clientesData.data?.[0] || null,
                productos: productosData.data?.[0] || null,
                anios: aniosData.data?.[0] || null,
                agencias: agenciasData.data?.[0] || null
            };

            console.log('Datos de campaña:', campanaConRelaciones);
            setCampana(campanaConRelaciones);
        } catch (error) {
    ...
    const fetchTemas = async () => {
        try {
            ...
            // Luego obtener los detalles de los temas
            ...
            const [mediosData, calidadesData] = await Promise.all([
                supabase.from('medios').select('id, nombredelmedio'),
    ...
    const fetchAlternativaData = async () => {
        try {
            // Primero obtener los planes de esta campaña
            ...
            const planIds = planesData.map(plan => plan.id);

            // Obtener las relaciones plan_alternativas para todos los planes de esta campaña
            ...
            // Obtener los IDs de las alternativas
            const alternativaIds = planAlternativas.map(pa => pa.id_alternativa);

            // Filtrar solo las alternativas que tienen orden creada (tienen numerorden)
            const { data, error } = await supabase
                .from('alternativa')
                .select('*')
                .in('id', alternativaIds)
                .not('numerorden', 'is', null)
                .not('numerorden', 'eq', 0);
            ...
            // Obtener datos relacionados por separado para cada alternativa
            const alternativaConRelaciones = await Promise.all(
                data.map(async (item) => {
                    let contrato = null;
                    let tema = null;
                    let anio = null;
                    ...
                    if (item.id_campania) {
                        const { data: campaniaData } = await supabase
                            ...
                            .eq('id_campania', item.id_campania)
                            .single();
                    ...
                    // Obtener soporte
                    if (item.id_soporte) {
                        const { data: soporteData } = await supabase
                            .from('soportes')
                            .select('id_soporte, nombreidentificador')
                            .eq('id_soporte', item.id_soporte)
                            .single();
                        soporte = soporteData;
                    }
            ...
            setAlternativaData(alternativaConRelaciones);
        } catch (error) {
            console.error('Error fetching alternativa data:', error);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No se pudieron cargar las órdenes de contrato: ' + error.message
            });
        }
    };
    ...
    const handleToggleTemaEstado = async (tema) => {
        try {
            ...
            const nuevoEstado = !tema.estado;
            
            // Mostrar SweetAlert de confirmación
            const result = await Swal.fire({
                title: '¿Cambiar estado?',
                text: `¿Desea ${tema.estado ? "desactivar" : "activar"} el tema "${tema.nombre_tema}"?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                ...
                await Swal.fire({
                    ...
                    text: `El tema ha sido ${nuevoEstado ? "activado" : "desactivado"} exitosamente`,
                    icon: 'success',
                    timer: 1500,
            ...
            <Grid container spacing={3}>
                ...
                <Paper elevation={3} sx={{ p: 3, mb: 3, textAlign: 'center' }}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                        Registrado el: {campana?.created_at ? new Date(campana.created_at).toLocaleDateString() : new Date().toLocaleDateString()}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                                    ...
                                    <TableBody>
                                        {alternativaData.map((row) => (
                                            <TableRow key={row.id}>
                                                ...
                                                <TableCell>{row.Temas?.nombre_tema}</TableCell>
                                                ...
                                                <TableCell>{row.Soportes?.nombreidentificador}</TableCell>
