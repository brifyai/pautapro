...
const ModalAgregarContrato = ({ open, onClose, onContratoAdded, clienteId, clienteNombre, disableClienteSelect }) => {
    const initialFormData = {
        nombrecontrato: '', // Normalizado a minúsculas para consistencia
        IdCliente: clienteId || '',
        IdProveedor: '',
        idmedios: '', // Normalizado para consistencia con el backend
        FechaInicio: '',
        FechaTermino: '',
        Estado: 'Vigente',
        id_FormadePago: '',
    ...
    const handleSubmit = async () => {
        ...
        const camposRequeridos = [
            ...
            'IdCliente',
            'IdProveedor',
            'idmedios', // Corregido para coincidir con el formData
            'FechaInicio',
            'FechaTermino',
            'id_FormadePago',
            'id_GeneraracionOrdenTipo'
        ...
        try {
            ...
            const datosNormalizados = {
                ...formData,
                NombreContrato: formData.nombrecontrato, // Mantener compatibilidad
                IdMedios: formData.idmedios // Mantener compatibilidad
            };
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Error al guardar el contrato. Por favor, intente nuevamente.'
            });
        } finally {
            setLoading(false);
        }
                    ...
                    <Grid container spacing={2} sx={{ mt: 1 }}>
                        <Grid item xs={12}>
                            <TextField
                                ...
                                disabled={loading}
                                error={!!error && !formData.nombrecontrato}
                                helperText={!formData.nombrecontrato ? 'Campo requerido' : ''}
                                InputProps={{
                                    startAdornment: (
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextField
                                ...
                                required
                                margin="normal"
                                error={!!error && !formData.idmedios}
                                helperText={!formData.idmedios ? 'Campo requerido' : ''}
                                InputProps={{
                                    startAdornment: (
                                        <InputAdornment position="start">
                                            <CategoryIcon />
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextField
                                ...
                                margin="normal"
                                error={!!error && !formData.IdProveedor}
                                helperText={!formData.IdProveedor ? 'Campo requerido' : ''}
                                InputProps={{
                                    startAdornment: (
                                        <InputAdornment position="start">
                                            <StorefrontIcon />
