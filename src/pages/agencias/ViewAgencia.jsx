...
function ViewAgencia() {
  ...
  const fetchRegionesyComunas = async () => {
    try {
      const { data: regionesData } = await supabase.from('region').select('*');
      const { data: comunasData } = await supabase.from('comunas').select('*');
      ...
      console.error('Error fetching regiones y comunas:', error);
    }
  };
  ...
  const fetchAgencia = async () => {
    try {
      setLoading(true);
      const { data: agenciaData, error } = await supabase
        .from('agencias')
        .select('*')
        .eq('id', id)
        .single();
      ...
      console.error('Error fetching agencia:', error);
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      // Finalmente recargamos los datos
      fetchAgencia();
