...
const ReporteOrdenDeCompra = () => {
  ...
  const generateReport = async () => {
    try {
      ...
      switch (reportType) {
        case 'daily_orders':
          reportData = await reportService.generateDailyOrdersReport();
          break;
        case 'weekly_performance':
          reportData = await reportService.generateWeeklyPerformanceReport();
          break;
        case 'monthly_revenue':
          reportData = await reportService.generateMonthlyRevenueReport();
          break;
        case 'campaign_status':
          reportData = await reportService.generateCampaignStatusReport();
          break;
        default:
      ...
      const excelFile = reportService.exportToExcel(reportData, reportType);
      const pdfFile = reportService.exportToPDF(reportData, reportType);
  ...
  const scheduleReport = async () => {
    try {
      ...
      await reportService.scheduleReport(reportType, config);
      ...
      alert('Error al programar el reporte: ' + error.message);
    }
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
        <Typography variant="h6" gutterBottom sx={{ fontWeight: 'medium', color: '#2c3e50', mb: 3 }}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              ...
              >
                <MenuItem value="daily_orders">Órdenes Diarias</MenuItem>
                <MenuItem value="weekly_performance">Rendimiento Semanal</MenuItem>
                <MenuItem value="monthly_revenue">Ingresos Mensuales</MenuItem>
                <MenuItem value="campaign_status">Estado de Campañas</MenuItem>
              </Select>
            </FormControl>
          </Grid>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              {scheduledReports.map((report) => (
                <Chip
                  key={report.id}
                  label={`${reportService.scheduledReports[report.report_type]?.name || report.report_type} (${report.frequency})`}
                  color="primary"
                  variant="outlined"
                  size="small"
                />
        ...
        <Alert severity="info" sx={{ mt: 3 }}>
          ...
          <Typography variant="body2" sx={{ mt: 1 }}>
            • <strong>Órdenes Diarias:</strong> Resumen de órdenes creadas en el día
          </Typography>
          <Typography variant="body2">
            • <strong>Rendimiento Semanal:</strong> Análisis de crecimiento y efectividad
          </Typography>
          <Typography variant="body2">
            • <strong>Ingresos Mensuales:</strong> Desglose detallado por semana y cliente
          </Typography>
          <Typography variant="body2">
            • <strong>Estado de Campañas:</strong> Seguimiento de campañas activas y vencidas
          </Typography>
        </Alert>
      </Paper>

      <Typography variant="h5" component="h2" gutterBottom sx={{ fontWeight: 'bold', color: '#1976d2', mb: 3 }}>
        Reporte Manual de Órdenes
      </Typography>
                ...
                <TableBody>
                  {paginatedOrdenes.map((orden) => (
                    <TableRow key={orden.id_ordenes_de_comprar}>
                      ...
                      <TableCell>{orden.Soportes?.nombreidentificador || 'NA'}</TableCell>
