document.addEventListener('DOMContentLoaded', function() {
    const SUPABASE_URL = 'https://rfjbsoxkgmuehrgteljq.supabase.co';
    ...
    async function actualizarestadoProveedor(temaID, nuevoEstado, toggleElement) {
        console.log(`Actualizando Tema - ID: ${temaID}, Nuevo estado: ${nuevoEstado}`);
        try {
            const response = await fetch(`${SUPABASE_URL}/rest/v1/Temas?id_tema=eq.${temaID}`, {
                method: 'PATCH',
                headers: {
            ...
            if (label) {
                label.setAttribute('title', nuevoEstado ? 'Desactivar Tema' : 'Activar Tema');
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No se pudo actualizar el estado del Tema: ' + error.message
            });
        }
    }
    ...
    document.addEventListener('change', function(event) {
        if (event.target.matches('.estado-switch2')) {
            const temaID = event.target.getAttribute('data-id');
            const nuevoEstado = event.target.checked;
            
            console.log(`Toggle cambiado - Tema ID: ${temaID}, Nuevo estado: ${nuevoEstado}`);
            ...
            Swal.fire({
                title: '¿Estás seguro?',
                text: `¿Deseas ${nuevoEstado ? 'activar' : 'desactivar'} este Tema?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    actualizarestadoProveedor(temaID, nuevoEstado, event.target);
                } else {
