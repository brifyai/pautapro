document.addEventListener('DOMContentLoaded', function() {
    ...
    async function actualizarestadoProveedor(proveedorID, nuevoEstado, toggleElement) {
        console.log(`Actualizando Proveedor - ID: ${proveedorID}, Nuevo estado: ${nuevoEstado}`);
        try {
            const response = await fetch(`${SUPABASE_URL}/rest/v1/Proveedores?id_proveedor=eq.${proveedorID}`, {
                method: 'PATCH',
                headers: {
            ...
            if (label) {
                label.setAttribute('title', nuevoEstado ? 'Desactivar Proveedor' : 'Activar Proveedor');
            ...
            Swal.fire({
                ...
                text: `El Proveedor ha sido ${nuevoEstado ? 'activado' : 'desactivado'} exitosamente.`,
    ...
    document.querySelectorAll('.estado-switch2').forEach(toggleSwitch => {
        toggleSwitch.addEventListener('change', function(event) {
            const proveedorID = this.getAttribute('data-id');
            const nuevoEstado = this.checked;
            
            console.log(`Toggle cambiado - Proveedor ID: ${proveedorID}, Nuevo estado: ${nuevoEstado}`);
            ...
            Swal.fire({
                title: '¿Estás seguro?',
                text: `¿Deseas ${nuevoEstado ? 'activar' : 'desactivar'} este Proveedor?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    actualizarestadoProveedor(proveedorID, nuevoEstado, this);
                } else {
