document.addEventListener('DOMContentLoaded', function() {
    ...
    async function actualizarestadoContrato(contratoID, nuevoEstado, toggleElement) {
        console.log(`Actualizando Contrato - ID: ${contratoID}, Nuevo estado: ${nuevoEstado}`);
        try {
            ...
            if (label) {
                label.setAttribute('title', nuevoEstado ? 'Desactivar Contrato' : 'Activar Contrato');
    ...
    document.querySelectorAll('.estado-switchC').forEach(toggleSwitch => {
        toggleSwitch.addEventListener('change', function(event) {
            const contratoID = this.getAttribute('data-id');
            const nuevoEstado = this.checked;
            
            console.log(`Toggle cambiado - Contrato ID: ${contratoID}, Nuevo estado: ${nuevoEstado}`);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    actualizarestadoContrato(contratoID, nuevoEstado, this);
                } else {
