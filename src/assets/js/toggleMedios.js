document.addEventListener('DOMContentLoaded', function() {
    ...
    async function actualizarestadoContrato(medioID, nuevoEstado, toggleElement) {
        console.log(`Actualizando Medio - ID: ${medioID}, Nuevo estado: ${nuevoEstado}`);
        try {
            const response = await fetch(`${SUPABASE_URL}/rest/v1/Medios?id=eq.${medioID}`, {
                method: 'PATCH',
                headers: {
            ...
            if (label) {
                label.setAttribute('title', nuevoEstado ? 'Desactivar Medio' : 'Activar Medio');
    ...
    document.querySelectorAll('.estado-switchM').forEach(toggleSwitch => {
        toggleSwitch.addEventListener('change', function(event) {
            const medioID = this.getAttribute('data-id');
            const nuevoEstado = this.checked;
            
            console.log(`Toggle cambiado - Medio ID: ${medioID}, Nuevo estado: ${nuevoEstado}`);
            ...
            Swal.fire({
                title: '¿Estás seguro?',
                text: `¿Deseas ${nuevoEstado ? 'activar' : 'desactivar'} este Medio?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    actualizarestadoContrato(medioID, nuevoEstado, this);
                } else {
